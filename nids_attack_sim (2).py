...
# Also discover other LAN hosts to simulate internal attacker
def _get_lan_targets():
    ...
    for i in range(1, 20):
        ...
        if ip != my_ip:
            targets.append(ip)
    return targets[:5]   # first 5 hosts on subnet

LAN_TARGETS = _get_lan_targets()
print(f'[*] Attack simulator — this machine: {TARGET_IP}')
print(f'[*] LAN targets for internal attack simulation: {LAN_TARGETS}')
...
def attack_lan_scan(subnet=None, port_range=(1, 1024)):
    """
    Simulates an internal LAN attacker doing reconnaissance.
    This is the most common internal threat — a compromised device
    scanning the local network for vulnerable hosts.
    """
    import socket
    my_ip = TARGET_IP
    subnet = subnet or '.'.join(my_ip.split('.')[:3])

    log(Y, 'LAN SCAN', f'Internal network scan starting on {subnet}.0/24')
    log(Y, 'LAN SCAN', 'Simulating: compromised LAN device doing reconnaissance')
    log(Y, 'LAN SCAN', 'Your NIDS should detect: PortScan from internal IP')

    found = []
    for host_num in range(1, 30):
        target = f'{subnet}.{host_num}'
        if target == my_ip:
            continue

        # Ping sweep
        alive = os.system(f'ping -c 1 -W 1 {target} > /dev/null 2>&1') == 0
        if alive:
            found.append(target)
            log(G, 'LAN SCAN', f'{target} is UP — scanning ports...')

            # Port scan the alive host
            for port in [21, 22, 23, 80, 443, 445, 3306, 3389, 8080]:
                try:
                    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    s.settimeout(0.3)
                    result = s.connect_ex((target, port))
                    if result == 0:
                        log(G, 'LAN SCAN', f'  {target}:{port} OPEN')
                    s.close()
                except Exception:
                    pass
                time.sleep(0.02)

    log(Y, 'LAN SCAN', f'LAN scan complete. Found {len(found)} hosts: {found}')
    return found


def attack_lan_bruteforce(target=None, port=22, attempts=60):
    """
    Simulates SSH brute force FROM a LAN IP — internal attacker.
    """
    import socket
    target = target or TARGET_IP
    log(M, 'LAN BRUTE', f'Internal SSH brute force: {TARGET_IP} -> {target}:{port}')
    log(M, 'LAN BRUTE', 'Your NIDS should detect: BruteForce from LAN IP')

    passwords = ['admin','root','password','123456','toor','kali','ubuntu',
                 'raspberry','letmein','test','guest','admin123','pass']
    users     = ['root','admin','user','ubuntu','pi','kali']
    ...
    for i in range(attempts):
        ...
        try:
            ...
            pass
        if (i+1) % 10 == 0:
            log(M, 'LAN BRUTE', f'Attempt {i+1}/{attempts} | user: {user}')
        time.sleep(0.03)

    log(M, 'LAN BRUTE', f'LAN brute force complete — {attempts} attempts sent')


def attack_lan_dos(target=None, port=80, duration=12):
    """
    Simulates DoS attack from inside the LAN.
    """
    import socket, threading
    target = target or TARGET_IP
    log(R, 'LAN DoS', f'Internal DoS: {TARGET_IP} -> {target}:{port} for {duration}s')
    log(R, 'LAN DoS', 'Your NIDS should detect: DoS Hulk from LAN IP')
    ...
    while time.time() - start < duration:
        time.sleep(2)
        log(R, 'LAN DoS', f'Sent {sent[0]:,} requests | {time.time()-start:.0f}s/{duration}s')

    stop[0] = True
    log(R, 'LAN DoS', f'LAN DoS complete — {sent[0]:,} total requests')


def run_lan_attacks(target=None):
    """Run all LAN-based internal attack simulations."""
    target = target or TARGET_IP
    log(R, 'LAN ALL', f'Starting full INTERNAL network attack simulation')
    log(R, 'LAN ALL', f'Attacker IP (this machine): {TARGET_IP}')
    log(R, 'LAN ALL', f'Target: {target}')
    log(R, 'LAN ALL', 'Watch NIDS dashboard for LAN detections!')
    print()

    attacks = [
        ('LAN Port Scan',    lambda: attack_lan_scan()),
        ('LAN SSH BruteForce', lambda: attack_lan_bruteforce(target, 22, 60)),
        ('LAN DoS Attack',   lambda: attack_lan_dos(target, 80, 12)),
        ('LAN UDP Flood',    lambda: attack_udp_flood(target, 53, 8)),
    ]

    for name, fn in attacks:
        print(f'\n{"─"*55}')
        log(R, 'LAN', f'Starting: {name}')
        print(f'{"─"*55}')
        try:
            fn()
        except Exception as e:
            log(R, 'LAN', f'{name} error: {e}')
        log(G, 'LAN', f'{name} done — pausing 3s...')
        time.sleep(3)

    log(G, 'LAN ALL', 'All LAN attacks complete! Check your NIDS dashboard.')
...
def interactive_menu():
    ...
    print(f'  --- EXTERNAL ATTACKS (from internet) ---')
    ...
    print(f'  {G}8{RS}. Run ALL external      — Full external attack simulation')
    print()
    print(f'  --- INTERNAL LAN ATTACKS (from inside network) ---')
    print(f'  {Y}9{RS}. LAN Port Scan         — Internal device scanning LAN')
    print(f'  {M}10{RS}. LAN SSH Brute Force  — Internal device brute forcing SSH')
    print(f'  {R}11{RS}. LAN DoS Attack       — Internal device DoS attack')
    print(f'  {G}12{RS}. Run ALL LAN attacks  — Full internal attack simulation')
    print(f'  {R}0{RS}. Exit')
    ...
    if choice == '1':
        ...
        attempts = int(input('  Number of attempts [60]: ') or '60')
        attack_lan_bruteforce(target, attempts=attempts)

    elif choice == '11':
        dur = int(input('  Duration in seconds [12]: ') or '12')
        attack_lan_dos(target, duration=dur)

    elif choice == '12':
        run_lan_attacks(target)
