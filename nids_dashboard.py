...
# ─────────────────────────────────────────────
# WHITELIST — only THIS machine is protected
# All other LAN devices are fully monitored
# If a LAN device attacks you, it WILL be detected
# ─────────────────────────────────────────────
...
MY_IP = _get_my_ip()
print(f'[*] This machine IP: {MY_IP} (whitelisted)')
print('[*] LAN attack detection: ENABLED')

WHITELIST = {
    MY_IP,        # only protect this machine from self-blocking
    '127.0.0.1',
    '::1',
}
...
class NetworkFlow:
    def __init__(self, src_ip, dst_ip, src_port, dst_port, protocol):
        ...
        self.dst_port = dst_port   # used for sensitive port detection
...
def heuristic_detect(flow):
    """
    Detects attacks using traffic pattern analysis.
    Uses DIFFERENT thresholds for LAN vs Internet traffic:
      - LAN attacks are slower but still suspicious
      - Internet attacks are typically faster and more aggressive
    """
    ...
    total_pkts= tf + tb
    src_is_lan= is_lan_ip(flow.src_ip)

    # Need minimum packets
    if total_pkts < (3 if src_is_lan else 5):
        return 'BENIGN', 96.0

    if src_is_lan:
        # ── LAN Port Scan: RST = hitting closed ports ──
        if rst >= 3:
            return 'PortScan', 88.0
        # ── LAN Port Scan: many tiny packets, no reply ──
        if tf >= 5 and avg < 120 and tb == 0 and pps > 3:
            return 'PortScan', 82.0
        # ── LAN SYN Flood ──
        if syn >= 5 and tb == 0:
            return 'DDoS', 87.0
        # ── LAN Brute Force on sensitive ports ──
        if tf >= 8 and tb >= 4 and 20 < avg < 300 and flow.dst_port in (22,21,23,3389,445,1433,3306,5900):
            return 'BruteForce', 85.0
        # ── LAN Brute Force any port ──
        if tf >= 15 and tb >= 8 and 20 < avg < 300:
            return 'BruteForce', 76.0
        # ── LAN DoS ──
        if pps > 80 and dur > 2:
            return 'DoS Hulk', 80.0
        # ── LAN DDoS ──
        if pps > 50 and tb < tf * 0.05 and tf > 30:
            return 'DDoS', 83.0
        # ── LAN Infiltration / data exfiltration ──
        if dur > 15 and bwd_bytes > 200_000:
            return 'Infiltration', 74.0
        if fwd_bytes > 500_000 and tb == 0 and dur > 3:
            return 'Infiltration', 72.0
        # ── LAN Recon: many tiny packets ──
        if tf >= 10 and avg < 80 and tb == 0:
            return 'PortScan', 71.0

    else:
        # ── External DDoS / UDP Flood ──
        if pps > 500 and tb == 0 and tf > 100:
            return 'DDoS', 95.0
        # ── External SYN Flood ──
        if syn >= 20 and tb == 0 and tf > 20:
            return 'DDoS', 92.0
        # ── External DDoS ──
        if pps > 200 and tb < tf * 0.02:
            return 'DDoS', 90.0
        # ── External Port Scan ──
        if rst >= 10 and tf > 10:
            return 'PortScan', 87.0
        if tf >= 20 and avg < 100 and tb == 0 and pps > 10:
            return 'PortScan', 80.0
        # ── External Brute Force ──
        if tf >= 30 and tb >= 15 and 20 < avg < 200:
            return 'BruteForce', 78.0
        # ── External DoS Hulk ──
        if pps > 300 and dur > 2:
            return 'DoS Hulk', 82.0
        # ── External Infiltration ──
        if dur > 30 and bwd_bytes > 500_000:
            return 'Infiltration', 75.0
        if fwd_bytes > 1_000_000 and tb == 0 and dur > 5:
            return 'DDoS', 72.0
...
def analyse_flow(key, flow, final=False):
    ...
    print(f'[ALERT] {label} | {src_ip}:{key[2]} → {key[1]}:{key[3]} | {conf:.1f}% | {action}')
...
def process_packet(pkt):
    ...
    # Only skip pure loopback self-talk (127.x -> 127.x)
    # LAN traffic (192.168.x, 10.x, 172.x) is ALLOWED through
    if pkt.haslayer(IP):
        si = pkt[IP].src; di = pkt[IP].dst
        # Skip only if BOTH src AND dst are loopback
        if si.startswith('127.') and di.startswith('127.'):
            return
...
def capture_on_iface(iface):
    try:
        ...
        bpf = (
            ...
            'not (port 5000) and '
            'not (port 5001) and '
            # Only skip pure loopback-to-loopback (dashboard internal)
            'not (src host 127.0.0.1 and dst host 127.0.0.1)'
            # LAN traffic (192.168.x.x, 10.x.x.x) is NOT filtered
            # so attacks from other LAN devices ARE captured
        )
...
<header>
  ...
  <div class="hright">
    ...
    <div class="sbadge" id="b-ml"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;vertical-align:middle"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg>ML</div>
    <div class="sbadge" id="b-geo"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;vertical-align:middle"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>GeoIP</div>
...
<nav>
  <button class="tab active" onclick="switchTab('dashboard',this)">
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:6px;vertical-align:middle"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>DASHBOARD
  </button>
  <button class="tab" onclick="switchTab('packets',this)">
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:6px;vertical-align:middle"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>PACKET LOG
  </button>
  <button class="tab" onclick="switchTab('map',this)">
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:6px;vertical-align:middle"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>ATTACK MAP
  </button>
</nav>
 ...
 <div class="dbody">
  ...
  <div class="mgrid">
   ...
   <div class="panel">
    ...
    <div class="afeed" id="alert-feed"><div class="empty">MONITORING LIVE TRAFFIC — NO THREATS YET</div></div>
   </div>
  </div>
  <div class="bgrid">
   <div class="panel">
   ...
   <div class="panel">
    ...
    <div class="pbody" id="blk-panel"><div class="empty">NO IPs CURRENTLY BLOCKED</div></div>
   </div>
  </div>
 </div>
</div>
...
<div class="view" id="view-packets">
 <div class="plog-body">
  <div class="plog-toolbar">
   ...
   <span class="pause-badge" id="pause-badge">II PAUSED</span>
...
//  SYSTEM STATUS 
// SVG icon strings for badges
const svgWifi   = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;vertical-align:middle"><path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><line x1="12" y1="20" x2="12.01" y2="20"/></svg>`;
const svgBrain  = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;vertical-align:middle"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg>`;
const svgGlobe  = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;vertical-align:middle"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>`;
...
fetch('/api/system').then(r=>r.json()).then(d=>{
  ...
  ib.innerHTML=svgWifi+d.interface; ib.className='sbadge ok';
  document.getElementById('f-iface').textContent='Interface: '+d.interface;
  const mb=document.getElementById('b-ml');
  mb.innerHTML=(d.ml_ready?svgBrain:svgWarn)+(d.ml_ready?'ML: ON':'ML: HEURISTIC');
  mb.className='sbadge '+(d.ml_ready?'ok':'warn');
  const gb=document.getElementById('b-geo');
  gb.innerHTML=(d.geoip_ready?svgGlobe:svgWarn)+(d.geoip_ready?'GeoIP: ON':'GeoIP: OFF');
...
function updateBlockedPanel(blocked){
  ...
  if(!e.length){p.innerHTML='<div class="empty">NO IPs CURRENTLY BLOCKED</div>';return;}
...
function addAlert(a){
  ...
  d.className='ai';
  d.innerHTML=`
    <div class="aico ${isB?'blocked':'monitored'}">${isB?
...
if __name__=='__main__':
    print('='*58)
    print(' NIDS — East Africa Network Defense')
    print(f'    Interface  : {INTERFACE}')
    print(f'    ML Model   : {"LOADED" if ML_READY else " Heuristic mode"}')
    print(f'    GeoIP      : {"LOADED" if GEOIP_READER else " Run: python nids_geoip.py --setup"}')
